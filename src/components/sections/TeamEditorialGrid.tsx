...
export function TeamEditorialGrid({ faces, imageAltSuffix }: TeamEditorialGridProps) {
  return (
    <div className="mt-14 grid grid-cols-1 gap-12 sm:mt-16 sm:grid-cols-2 sm:gap-x-8 sm:gap-y-14 lg:mt-20 lg:grid-cols-3 lg:gap-x-10">
      {faces.map((face, index) => (
        <MotionReveal key={face.name} delay={0.08 + index * 0.06} className="group">
          <figure>
            <div className="relative aspect-[4/5] overflow-hidden rounded-[1.35rem] border border-walnut/[0.08] bg-sand/40 shadow-[0_14px_40px_rgba(58,44,34,0.08)] transition-shadow duration-700 ease-out group-hover:shadow-[0_18px_48px_rgba(58,44,34,0.12)] sm:rounded-[1.45rem]">
              <Image
                src={face.image}
            ...
            <figcaption className="mt-5 sm:mt-6">
              ...
              <p className={`mt-3 max-w-[18rem] text-walnut/90 ${bodyNoteClasses}`}>{face.quote}</p>
            </figcaption>
          </figure>
        </MotionReveal>
      ))}
